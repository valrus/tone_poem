...
class Context:
    ...
    def outBisector(self, edge, sitePair):
        ...
        if self.debug:
            ...
            if self.plot:
                self.line(
                    edge.reg[0].x, edge.reg[0].y, edge.reg[1].x, edge.reg[1].y
                )
        elif self.doPrint and not self.plot:
...
class Halfedge(object):
    ...
    def __lt__(self, other):
        return self.ystar < other.ystar or (
            self.ystar == other.ystar and self.vertex.x < other.vertex.x
        )
    ...
    def isPointRightOf(self, pt):
        ...
        if e.a == 1.0:
            ...
            dxp = pt.x - topsite.x
            fast = 0
            if (not right_of_site and e.b < 0.0) or (
...
class EdgeList(object):
    ...
    def leftbnd(self, pt):
        ...
        if (he is self.leftend) or (
            he is not self.rightend and he.isPointRightOf(pt)
        ):
            he = he.right
